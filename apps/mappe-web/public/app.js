...
init().catch((error) => {
  console.error(error);
  showAuthError(`No pude iniciar MapPe: ${error.message || error}`);
...
function buildMarketStudyWorkbook(rows) {
  ...
  summary.getCell('A1').value = 'MapPe · Estudio de mercado';
...
function inferDivisionType(row) { if (row.city === 'Lima') return 'distrito'; if (row.city === 'Bogotá') return 'localidad'; if (row.city === 'Medellín' || row.city === 'Cali') return 'comuna'; return 'territorio'; }
...
function renderAdminPanel() {
  ...
  if (!state.admin.users.length) {
    list.innerHTML = '<div class="admin-user-card"><strong>No hay usuarios todavía.</strong><span class="muted">Cuando alguien cree cuenta en MapPe aparecerá aquí.</span></div>';
    return;
  }
...
async function loadAdminUsers() {
  ...
  try {
    ...
    state.admin.error = error.message || 'No pude cargar usuarios de MapPe.';
  } finally {
