...
const OUTPUT_NAME = 'Cotizador_EVINKA_validacion_v4.xlsx';
...
async function buildWorkbook() {
  ...
  setPage(rs);
  titleBlock(rs, 'EVINKA · Cotizador de validación v4', 'Versión con crudo + margen separados y diseño base inspirado en Cotizador_EVINKA_Peru_v5.xlsx.');
  sectionBand(rs, 4, 'OBJETIVO');
  rs.mergeCells('A5:H6');
  rs.getCell('A5').value = 'Validar la lógica comercial sin doble margen. Ahora cada precio muestra crudo, margen y precio con margen por separado.';
  rs.getCell('A5').alignment = { wrapText: true, vertical: 'middle' };
  rs.getRow(5).height = 26;
  rs.getRow(6).height = 26;
  ...
  [
    ...
    'Crudo = precio base actual × 0.75.',
    'Margen = crudo × 0.25.',
    'Precio con margen = crudo × 1.25.',
    'El total final es la suma de totales con margen, ya sin aplicar /0.75 al final.'
  ].forEach((t, i) => {
  ...
  sectionBand(rs, 18, 'QUÉ REVISAR');
  tableHeader(rs, 19, ['Campo', 'Validación esperada', '', '', '', '', '', '']);
  [
    ['Crudo', 'Debe verse como valor separado en cada fila.'],
    ['Margen', 'Debe salir como columna aparte desde el crudo.'],
    ['Precio con margen', 'Debe salir por fórmula: crudo × 1.25.'],
    ['Total', 'Debe usar precio con margen, no volver a inflar al final.']
  ].forEach((row, i) => {
    rs.mergeCells(`B${20 + i}:H${20 + i}`);
    rs.getCell(`A${20 + i}`).value = row[0];
    rs.getCell(`B${20 + i}`).value = row[1];
    dataCell(rs.getCell(`A${20 + i}`));
    dataCell(rs.getCell(`B${20 + i}`));
  });
  zebra(rs, 20, 23, 1, 8);
  ...
  tableHeader(fv, 22, ['Código', 'Sección', 'Descripción', 'Activar', 'Cantidad', 'Precio base actual', 'Observación', '']);
  const conds = [
  ...
  setPage(cat);
  titleBlock(cat, 'CATÁLOGO · Base para validar obligatorios y condicionales', 'Ahora separa crudo y precio con margen para evitar doble margen.', 'J');
  tableHeader(cat, 4, ['Código', 'Sección', 'Naturaleza', 'Etiqueta', 'Unidad', 'Descripción', 'Crudo', 'Margen 25%', 'Precio con margen', 'Regla']);
  const catalogRows = [
  ...
  for (const row of catalogRows) {
    const [code, sec, nat, et, un, desc, precioBase, regla] = row;
    [code, sec, nat, et, un, desc].forEach((v, i) => { cat.getCell(cr, i + 1).value = v; dataCell(cat.getCell(cr, i + 1)); });
    cat.getCell(`G${cr}`).value = Number((precioBase * 0.75).toFixed(2));
    cat.getCell(`H${cr}`).value = { formula: `G${cr}*0.25`, result: Number((precioBase * 0.75 * 0.25).toFixed(2)) };
    cat.getCell(`I${cr}`).value = { formula: `G${cr}*1.25`, result: Number((precioBase * 0.75 * 1.25).toFixed(2)) };
    cat.getCell(`J${cr}`).value = regla;
    ['G','H','I'].forEach((col) => { dataCell(cat.getCell(`${col}${cr}`)); money(cat.getCell(`${col}${cr}`)); });
    dataCell(cat.getCell(`J${cr}`));
  ...
  cot.columns = [
    { width: 14 }, { width: 16 }, { width: 9 }, { width: 11 }, { width: 9 }, { width: 12 }, { width: 34 }, { width: 14 }, { width: 14 }, { width: 16 }, { width: 16 }, { width: 24 },
  ];
  setPage(cot);
  titleBlock(cot, 'Cálculo de Cotización', 'Salida ordenada con crudo, margen y precio con margen separados.', 'L');
  noteBand(cot, 4, 'Nueva lógica: crudo = precio base actual × 0.75. El precio con margen sale por fórmula: crudo × 1.25.', 'L');
  ...
  tableHeader(cot, 8, ['Sección','Naturaleza','Activo','Cantidad','Unidad','Código','Descripción','Crudo','Margen 25%','Precio con margen','Total','Regla']);
  const base = [
    ['MANO_OBRA','OBLIGATORIO',1,1,'ZZ','0060001','Servicio de instalación estándar',{formula:"IF('01_FORM_VISITA'!B11<25,525,IF('01_FORM_VISITA'!B11<40,525*1.4,IF('01_FORM_VISITA'!B11<50,525*2,525*3)))*0.75",result:551.25},'Factor por distancia'],
    ['MANO_OBRA','OBLIGATORIO',1,1,'ZZ','0060002','Visita técnica e ingeniería',82.5,'Siempre incluida'],
    ['MANO_OBRA','OBLIGATORIO',1,1,'ZZ','0060003','Transporte y herramientas',{formula:"IF('01_FORM_VISITA'!B11<25,120,IF('01_FORM_VISITA'!B11<40,120*1.4,IF('01_FORM_VISITA'!B11<50,120*2,120*3)))*0.75",result:126},'Factor por distancia'],
    ['MATERIAL','OBLIGATORIO',1,1,'UND','0060101','Tablero eléctrico 6p',123,'Siempre incluido'],
    ['MATERIAL','OBLIGATORIO',1,{formula:"'01_FORM_VISITA'!B11*1.1*2",result:77},'M','0060102','Cable 6mm',5.93,'Distancia × 1.1 × 2'],
    ['MATERIAL','OBLIGATORIO',1,{formula:"'01_FORM_VISITA'!B11*1.1",result:38.5},'M','0060103','Cable 4mm',4.2,'Distancia × 1.1'],
    ['MATERIAL','OBLIGATORIO_REGLA',{formula:"IF('01_FORM_VISITA'!B12=\"PVC\",1,0)",result:0},{formula:"IF(C15=1,ROUNDUP('01_FORM_VISITA'!B11/3,0),0)",result:0},'UND','0060104','Tubería PVC',18.53,'Solo si tubería = PVC'],
    ['MATERIAL','OBLIGATORIO_REGLA',{formula:"IF('01_FORM_VISITA'!B12=\"EMT\",1,0)",result:1},{formula:"IF(C16=1,ROUNDUP('01_FORM_VISITA'!B11/3,0),0)",result:12},'UND','0060105','Tubería EMT 3m 3/4',13.13,'Solo si tubería = EMT'],
    ['MATERIAL','OBLIGATORIO',1,1,'UND','0060106','Interruptor termomagnético',{formula:"IF('01_FORM_VISITA'!B9=\"Monofásico\",IF('01_FORM_VISITA'!B10=\"B2C\",26.7,58.7),IF('01_FORM_VISITA'!B10=\"B2C\",46.7,109.4))*0.75",result:20.03},'Depende de instalación/cliente'],
    ['MATERIAL','OBLIGATORIO',1,1,'UND','0060107','Interruptor diferencial',{formula:"IF('01_FORM_VISITA'!B10=\"B2C\",105.4,424)*0.75",result:79.05},'Depende de tipo cliente'],
    ['MATERIAL','OBLIGATORIO_REGLA',{formula:"IF('01_FORM_VISITA'!B12=\"EMT\",1,0)",result:1},{formula:"IF(C19=1,ROUNDUP('01_FORM_VISITA'!B11/3,0),0)",result:12},'UND','0060108','Accesorios EMT Conduit',2.63,'Solo si tubería = EMT'],
  ];
  let dr = 9;
  for (const row of base) {
    const [sec, nat, act, cant, un, code, desc, crudo, regla] = row;
    [sec, nat, act, cant, un, code, desc].forEach((v, i) => { cot.getCell(dr, i + 1).value = v; dataCell(cot.getCell(dr, i + 1)); });
    cot.getCell(`H${dr}`).value = crudo; dataCell(cot.getCell(`H${dr}`));
    cot.getCell(`I${dr}`).value = { formula: `H${dr}*'01_FORM_VISITA'!B17`, result: 0 }; dataCell(cot.getCell(`I${dr}`));
    cot.getCell(`J${dr}`).value = { formula: `H${dr}*1.25`, result: 0 }; dataCell(cot.getCell(`J${dr}`));
    cot.getCell(`K${dr}`).value = { formula: `C${dr}*D${dr}*J${dr}`, result: 0 }; dataCell(cot.getCell(`K${dr}`));
    cot.getCell(`L${dr}`).value = regla; dataCell(cot.getCell(`L${dr}`));
    qty(cot.getCell(`D${dr}`));
    ['H', 'I', 'J', 'K'].forEach((col) => money(cot.getCell(`${col}${dr}`)));
  ...
  for (let i = 0; i < map.length; i += 1) {
    ...
    cot.getCell(`E${tr}`).value = units[i];
    cot.getCell(`F${tr}`).value = { formula: `'01_FORM_VISITA'!A${fr}`, result: '' };
    cot.getCell(`G${tr}`).value = { formula: `'01_FORM_VISITA'!C${fr}`, result: '' };
    cot.getCell(`H${tr}`).value = { formula: `'01_FORM_VISITA'!F${fr}*0.75`, result: 0 };
    cot.getCell(`I${tr}`).value = { formula: `H${tr}*'01_FORM_VISITA'!B17`, result: 0 };
    cot.getCell(`J${tr}`).value = { formula: `H${tr}*1.25`, result: 0 };
    cot.getCell(`K${tr}`).value = { formula: `C${tr}*D${tr}*J${tr}`, result: 0 };
    cot.getCell(`L${tr}`).value = 'Solo suma si técnico lo activa';
    for (const a of [`A${tr}`,`B${tr}`,`C${tr}`,`D${tr}`,`E${tr}`,`F${tr}`,`G${tr}`,`H${tr}`,`I${tr}`,`J${tr}`,`K${tr}`,`L${tr}`]) dataCell(cot.getCell(a));
    qty(cot.getCell(`D${tr}`));
    ['H', 'I', 'J', 'K'].forEach((col) => money(cot.getCell(`${col}${tr}`)));
  ...
  cot.getCell('H38').value = { formula: 'ROUND(SUM(K26:K29)*0.4,2)*0.75', result: 0 }; dataCell(cot.getCell('H38'));
  cot.getCell('I38').value = { formula: `H38*'01_FORM_VISITA'!B17`, result: 0 }; dataCell(cot.getCell('I38'));
  cot.getCell('J38').value = { formula: 'H38*1.25', result: 0 }; dataCell(cot.getCell('J38'));
  cot.getCell('K38').value = { formula: 'C38*D38*J38', result: 0 }; dataCell(cot.getCell('K38'));
  cot.getCell('L38').value = '40% de MO civil activa'; dataCell(cot.getCell('L38'));
  ['H', 'I', 'J', 'K'].forEach((col) => money(cot.getCell(`${col}38`)));
  zebra(cot, 9, 38, 1, 12);
  ...
  cot.getCell('H42').value = 'SUBTOTAL FINAL'; dataCell(cot.getCell('H42'),'dark-total');
  cot.getCell('K42').value = { formula: 'K40+K41', result: 0 }; dataCell(cot.getCell('K42'),'dark-total'); money(cot.getCell('K42'));
  cot.getCell('H44').value = 'TOTAL CRUDO'; dataCell(cot.getCell('H44'),'gold-total');
  ...
  cot.getCell('H46').value = 'TOTAL CON MARGEN'; dataCell(cot.getCell('H46'),'gold-total');
