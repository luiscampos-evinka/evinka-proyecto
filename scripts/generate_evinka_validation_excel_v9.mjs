...
const OUTPUT_NAME = 'Cotizador_EVINKA_validacion_v9.xlsx';
...
const factorCostFormula = (row) => `IF($E$6<='00_PARAMETROS'!A6,VLOOKUP(F${row},'02_CATALOGO'!A:K,8,FALSE)*'00_PARAMETROS'!B6,IF($E$6<='00_PARAMETROS'!A7,VLOOKUP(F${row},'02_CATALOGO'!A:K,8,FALSE)*'00_PARAMETROS'!B7,IF($E$6<='00_PARAMETROS'!A8,VLOOKUP(F${row},'02_CATALOGO'!A:K,8,FALSE)*'00_PARAMETROS'!B8,IF($E$6<='00_PARAMETROS'!A9,VLOOKUP(F${row},'02_CATALOGO'!A:K,8,FALSE)*'00_PARAMETROS'!B9,VLOOKUP(F${row},'02_CATALOGO'!A:K,8,FALSE)*'00_PARAMETROS'!B10))))`;
const factorPriceFormula = (row) => `IF($E$6<='00_PARAMETROS'!A6,VLOOKUP(F${row},'02_CATALOGO'!A:K,10,FALSE)*'00_PARAMETROS'!B6,IF($E$6<='00_PARAMETROS'!A7,VLOOKUP(F${row},'02_CATALOGO'!A:K,10,FALSE)*'00_PARAMETROS'!B7,IF($E$6<='00_PARAMETROS'!A8,VLOOKUP(F${row},'02_CATALOGO'!A:K,10,FALSE)*'00_PARAMETROS'!B8,IF($E$6<='00_PARAMETROS'!A9,VLOOKUP(F${row},'02_CATALOGO'!A:K,10,FALSE)*'00_PARAMETROS'!B9,VLOOKUP(F${row},'02_CATALOGO'!A:K,10,FALSE)*'00_PARAMETROS'!B10))))`;
...
async function buildWorkbook() {
  ...
  p.getCell('A14').value='Factor de costos'; dataCell(p.getCell('A14'),'label'); p.getCell('B14').value='Se movió a 02_CATALOGO'; dataCell(p.getCell('B14')); p.getCell('C14').value='Ahora el cotizador jala el costo ajustado desde el catálogo'; dataCell(p.getCell('C14'));
  ...
  setPage(cat); titleBlock(cat,'CATÁLOGO · Maestro único de precios','Edita COSTO BASE y el factor aquí; el cotizador jala el costo ajustado.','K');
  cat.getCell('G3').value='Factor general de costos'; dataCell(cat.getCell('G3'),'label'); cat.getCell('H3').value=1; dataCell(cat.getCell('H3'),'input'); cat.mergeCells('I3:K3'); cat.getCell('I3').value='Este factor vive en el catálogo y afecta el costo ajustado que consume el cotizador'; dataCell(cat.getCell('I3'));
  tableHeader(cat,4,['Código','Sección','Naturaleza','Etiqueta','Unidad','Descripción','Costo base','Costo ajustado','Margen','Precio con margen','Regla']);
  ...
  setPage(cot); titleBlock(cot,'Cálculo de Cotización','El cotizador jala costos ajustados del catálogo y aplica factores/calibres configurables.','L'); noteBand(cot,4,'Servicio estándar y Transporte usan factor por distancia. El cable principal usa 6/10/16mm según límites. El factor general de costos ahora vive en 02_CATALOGO.','L');
