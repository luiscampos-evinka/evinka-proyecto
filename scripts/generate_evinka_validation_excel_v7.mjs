...
const OUTPUT_NAME = 'Cotizador_EVINKA_validacion_v7.xlsx';
...
['0060102','MATERIAL','OBLIGATORIO','Sí','M','Cable 6mm',5.93,'Cantidad = distancia × 1.1 × 2'],
...
['MATERIAL','OBLIGATORIO',1,{formula:"'01_FORM_VISITA'!B11*1.1*2",result:22},'M','0060102','Cable 6mm','Distancia × 1.1 × 2'],
...
const factorCostFormula = (row) => `IF($E$6<='00_PARAMETROS'!A5,VLOOKUP(F${row},'02_CATALOGO'!A:J,7,FALSE)*'00_PARAMETROS'!B5,IF($E$6<='00_PARAMETROS'!A6,VLOOKUP(F${row},'02_CATALOGO'!A:J,7,FALSE)*'00_PARAMETROS'!B6,IF($E$6<='00_PARAMETROS'!A7,VLOOKUP(F${row},'02_CATALOGO'!A:J,7,FALSE)*'00_PARAMETROS'!B7,IF($E$6<='00_PARAMETROS'!A8,VLOOKUP(F${row},'02_CATALOGO'!A:J,7,FALSE)*'00_PARAMETROS'!B8,VLOOKUP(F${row},'02_CATALOGO'!A:J,7,FALSE)*'00_PARAMETROS'!B9))))`;
const factorPriceFormula = (row) => `IF($E$6<='00_PARAMETROS'!A5,VLOOKUP(F${row},'02_CATALOGO'!A:J,9,FALSE)*'00_PARAMETROS'!B5,IF($E$6<='00_PARAMETROS'!A6,VLOOKUP(F${row},'02_CATALOGO'!A:J,9,FALSE)*'00_PARAMETROS'!B6,IF($E$6<='00_PARAMETROS'!A7,VLOOKUP(F${row},'02_CATALOGO'!A:J,9,FALSE)*'00_PARAMETROS'!B7,IF($E$6<='00_PARAMETROS'!A8,VLOOKUP(F${row},'02_CATALOGO'!A:J,9,FALSE)*'00_PARAMETROS'!B8,VLOOKUP(F${row},'02_CATALOGO'!A:J,9,FALSE)*'00_PARAMETROS'!B9))))`;
...
async function buildWorkbook() {
  ...
  sectionBand(p,12,'REGLA DE MARGEN'); p.getCell('A13').value='Divisor precio con margen'; dataCell(p.getCell('A13'),'label'); p.getCell('B13').value=0.75; dataCell(p.getCell('B13'),'input'); p.getCell('C13').value='Precio con margen = costo / este valor'; dataCell(p.getCell('C13')); pct(p.getCell('B13'));
  ...
  setPage(cot); titleBlock(cot,'Cálculo de Cotización','El cotizador jala costos del catálogo y aplica factores configurables.','L'); noteBand(cot,4,'Servicio de instalación estándar y Transporte usan factor por distancia configurable en 00_PARAMETROS.','L');
  ...
  let dr=9; for (const row of BASE_ROWS) { const [sec,nat,act,qv,un,code,desc,rule]=row; [sec,nat,act,qv,un,code,desc].forEach((v,i)=>{ cot.getCell(dr,i+1).value=v; dataCell(cot.getCell(dr,i+1)); }); if (code==='0060001' || code==='0060003') { cot.getCell(`H${dr}`).value={formula:factorCostFormula(dr),result:0}; cot.getCell(`J${dr}`).value={formula:factorPriceFormula(dr),result:0}; cot.getCell(`I${dr}`).value={formula:`J${dr}-H${dr}`,result:0}; } else { cot.getCell(`H${dr}`).value={formula:`VLOOKUP(F${dr},'02_CATALOGO'!A:J,7,FALSE)`,result:0}; cot.getCell(`I${dr}`).value={formula:`VLOOKUP(F${dr},'02_CATALOGO'!A:J,8,FALSE)`,result:0}; cot.getCell(`J${dr}`).value={formula:`VLOOKUP(F${dr},'02_CATALOGO'!A:J,9,FALSE)`,result:0}; } cot.getCell(`K${dr}`).value={formula:`C${dr}*D${dr}*J${dr}`,result:0}; cot.getCell(`L${dr}`).value=rule; dataCell(cot.getCell(`L${dr}`)); ['H','I','J','K'].forEach(col=>{ dataCell(cot.getCell(`${col}${dr}`)); money(cot.getCell(`${col}${dr}`)); }); qty(cot.getCell(`D${dr}`)); dr++; }

  const units = ['ZZ','ZZ','ZZ','ZZ','ZZ','ZZ','ZZ','ZZ','ML','ML','ZZ','ZZ','UND','UND','UND','UND','UND','UND'];
  for (let i=0;i<18;i++) { const fr=23+i; const tr=20+i; cot.getCell(`A${tr}`).value=i<12?'MANO_OBRA':'MATERIAL'; dataCell(cot.getCell(`A${tr}`)); cot.getCell(`B${tr}`).value='CONDICIONAL'; dataCell(cot.getCell(`B${tr}`)); cot.getCell(`C${tr}`).value={formula:`'01_FORM_VISITA'!D${fr}`,result:0}; dataCell(cot.getCell(`C${tr}`)); cot.getCell(`D${tr}`).value={formula:`IF(C${tr}=1,'01_FORM_VISITA'!E${fr},0)`,result:0}; dataCell(cot.getCell(`D${tr}`)); cot.getCell(`E${tr}`).value=units[i]; dataCell(cot.getCell(`E${tr}`)); cot.getCell(`F${tr}`).value={formula:`'01_FORM_VISITA'!A${fr}`,result:''}; dataCell(cot.getCell(`F${tr}`)); cot.getCell(`G${tr}`).value={formula:`'01_FORM_VISITA'!C${fr}`,result:''}; dataCell(cot.getCell(`G${tr}`)); cot.getCell(`H${tr}`).value={formula:`IFERROR(VLOOKUP(F${tr},'02_CATALOGO'!A:J,7,FALSE),0)`,result:0}; cot.getCell(`I${tr}`).value={formula:`IFERROR(VLOOKUP(F${tr},'02_CATALOGO'!A:J,8,FALSE),0)`,result:0}; cot.getCell(`J${tr}`).value={formula:`IFERROR(VLOOKUP(F${tr},'02_CATALOGO'!A:J,9,FALSE),0)`,result:0}; cot.getCell(`K${tr}`).value={formula:`C${tr}*D${tr}*J${tr}`,result:0}; cot.getCell(`L${tr}`).value='Solo suma si técnico lo activa'; dataCell(cot.getCell(`L${tr}`)); ['H','I','J','K'].forEach(col=>{ dataCell(cot.getCell(`${col}${tr}`)); money(cot.getCell(`${col}${tr}`)); }); qty(cot.getCell(`D${tr}`)); }

  cot.getCell('A38').value='MATERIAL'; dataCell(cot.getCell('A38')); cot.getCell('B38').value='CONDICIONAL_REGLA'; dataCell(cot.getCell('B38')); cot.getCell('C38').value={formula:'IF(SUM(K26:K29)>0,1,0)',result:0}; dataCell(cot.getCell('C38')); cot.getCell('D38').value=1; dataCell(cot.getCell('D38')); cot.getCell('E38').value='GL'; dataCell(cot.getCell('E38')); cot.getCell('F38').value='0060109'; dataCell(cot.getCell('F38')); cot.getCell('G38').value='Materiales obra civil'; dataCell(cot.getCell('G38')); cot.getCell('J38').value={formula:'ROUND(SUM(K26:K29)*0.4,2)',result:0}; cot.getCell('H38').value={formula:`J38*'00_PARAMETROS'!B13`,result:0}; cot.getCell('I38').value={formula:'J38-H38',result:0}; cot.getCell('K38').value={formula:'C38*D38*J38',result:0}; cot.getCell('L38').value='40% de MO civil activa'; dataCell(cot.getCell('L38')); ['H','I','J','K'].forEach(col=>{ dataCell(cot.getCell(`${col}38`)); money(cot.getCell(`${col}38`)); });
  zebra(cot,9,38,1,12);
